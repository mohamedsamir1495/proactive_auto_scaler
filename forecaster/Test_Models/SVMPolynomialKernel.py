...
class SVMPolynomialKernel(TestModel):
    def __init__(self):
        super().__init__("SVM Polynomial Kernel")
        self.model = None

    def train_model(self, train, column_name):
        svr_poly = SVR(kernel="poly", C=1.9, epsilon=0.027, coef0=1)
        X = np.arange(0, len(train), 1).reshape(-1, 1)
        X %= 24

        svr_fit = svr_poly.fit(X, train[column_name])
        self.model = svr_fit
